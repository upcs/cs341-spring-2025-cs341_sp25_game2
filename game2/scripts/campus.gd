...
func _on_library_entrance_body_shape_entered(body_rid: RID, body: Node2D, body_shape_index: int, local_shape_index: int) -> void:
	if body.is_in_group("Player") and (Global.markercount == 3 or Global.markercount>4):
		if not out_of_time:
			class_on_time = true
...
func _on_shiley_entrance_body_entered(body: Node2D) -> void:
	if body.is_in_group("Player") and (Global.markercount == 4 or Global.markercount >4):
		if not out_of_time:
			class_on_time = true
